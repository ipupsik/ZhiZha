...
void MeshResource::initMesh() {
	...
	{
		glBufferData(GL_ARRAY_BUFFER, sizeof(sf::Vector2f) * _vertices.size(), _vertices.data(), GL_STATIC_DRAW);
	}
	...
	{
		glBufferData(GL_ARRAY_BUFFER, sizeof(sf::Vector2f) * _uvs.size(), _uvs.data(), GL_STATIC_DRAW);
	}
	glBindBuffer(GL_ARRAY_BUFFER, 0);
	...
	{
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(sf::Vector3<unsigned int>) * _faces.size(), _faces.data(), GL_STATIC_DRAW);
	}
